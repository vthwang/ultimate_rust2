...
fn child_thread(handle: &str, rx: Receiver<i32>, tx: Sender<i32>) {
    for value in rx.iter() {
        if tx.send(value * value).is_err() {
            println!("Child thread {} error sending to channel.", handle);
            return;
        }
        println!("Child thread {} sent {}", handle, value * value);

        thread::sleep(Duration::from_secs_f32(1.0));
    }
    println!("Child thread {} done.", handle);
}
...
fn main() {
    ...
    // We will create a new unbounded channel; fork two threads each with a receiver.
    ...
    // Child Thread 1
    let handle1 = thread::spawn(move || { child_thread("handle 1", orders_rx, lunches_tx) });

    // Child Thread 2
    let handle2 = thread::spawn(move || { child_thread("handle 2", orders_rx2, lunches_tx2) });

    // Main thread sends values to the channel
    for num in 0..5 {
        println!("Main thread: Sending {}", num);
        let _ = orders_tx.send(num);
    }
